...
public C apply(final List<C> population) {
    ...
    if (population.size() < TOURNAMENT_SIZE) {
        throw new NoSuchElementException("Population size must be at least " + TOURNAMENT_SIZE);
    }

    // Shuffle the population to ensure randomness and distinct selection
    List<C> shuffledPopulation = new ArrayList<>(population);
    Collections.shuffle(shuffledPopulation, random);

    C candidate1 = shuffledPopulation.get(0);
    C candidate2 = shuffledPopulation.get(1);

    // Use comparator to select the fitter chromosome
    C selected = comparator.compare(candidate1, candidate2) < 0 ? candidate1 : candidate2;
