...
public class ShiftToBeginningMutation implements Mutation<TestOrder> {
    ...
    public TestOrder apply(TestOrder encoding) {
        ...
        // Ensure we select an index other than 0
        int indexToShift;
        do {
            indexToShift = random.nextInt(positions.length);
        } while (indexToShift == 0);
        ...
        return new TestOrder(encoding.getMutation(), positions.clone()); // Return deep copy to ensure immutability
    }
}
