...
public final class BranchTracer implements IBranchTracer {
    ...
    private final Map<Integer, Integer> approachLevels = new HashMap<>();

/**
 * Retrieves the approach level for the specified branch.
 *
 * @param branchID the ID of the branch
 * @return the approach level for the branch, or a default value (e.g., Integer.MAX_VALUE) if not tracked
 */
public int getApproachLevel(final int branchID) {
    return approachLevels.getOrDefault(branchID, Integer.MAX_VALUE); // Default value if not tracked
}
     ...
     */
    /**
 * Computes the branching distance for a conditional integer comparison with 0.
...
public void passedBranch(final int i, final int opcode, final int trueBranch, final int falseBranch) {
    ...
    // Compute distances based on the opcode
    switch (opcode) {
        case IFEQ: // i == 0
        ...
        case IFNE: // i != 0
            ...
            case IFLT: // i < 0
    distanceTrue = (i < 0) ? 0.0 : Math.abs(i) + 1.0;  // True branch distance
    distanceFalse = (i < 0) ? Math.abs(i) : 0.0;      // False branch distance
    break;
    ...
    case IFLE: // i <= 0
    distanceTrue = (i <= 0) ? 0.0 : i; // True branch distance
    distanceFalse = (i <= 0) ? Math.abs(i) + 1.0 : 0.0; // False branch distance
    break;


    case IFGT: // i > 0
    distanceTrue = (i > 0) ? 0.0 : Math.abs(i) + 1.0; // True branch distance
    distanceFalse = (i > 0) ? i : 0.0; // False branch distance
    break;
    ...
    // Record the branch distances if they improved
     ...
     */
   /**
 * Computes the branching distance for a conditional comparison of two integers.
 * Called by the instrumented code each time such a decision node is taken.
 *
 * @param i           the first integer to compare
 * @param j           the second integer to compare
...
public void passedBranch(final int i, final int j, final int opcode, final int trueBranch,
    ...
    switch (opcode) {
        ...
        case IF_ICMPLT: // i < j
            distanceTrue = (i < j) ? 0.0 : (i - j + 1.0);
            distanceFalse = (i < j) ? (j - i) : 0.0;
            break;

        case IF_ICMPLE: // i <= j
            distanceTrue = (i <= j) ? 0.0 : (i - j);
            distanceFalse = (i <= j) ? (j - i + 1.0) : 0.0;
            break;

        case IF_ICMPGT: // i > j
            distanceTrue = (i > j) ? 0.0 : (j - i + 1.0);
            distanceFalse = (i > j) ? (i - j) : 0.0;
            break;

        case IF_ICMPGE: // i >= j
            distanceTrue = (i >= j) ? 0.0 : (j - i);
            distanceFalse = (i >= j) ? (i - j + 1.0) : 0.0;
            break;
...
public void passedBranch(final Object o, final int opcode, final int trueBranch,
    ...
    private void traceBranchDistance(final int trueBranch, final double distanceTrue, final int falseBranch, final double distanceFalse) {
        // Update the distances map for the true branch
        distances.merge(trueBranch, distanceTrue, Math::min);
    
        // Update the distances map for the false branch
