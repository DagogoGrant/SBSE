...
public class AlgorithmBuilder {
    ...
    private GeneticAlgorithm<?> buildRandomSearch() {
        ...
        TestChromosomeMutation mutation = new TestChromosomeMutation(null); // Temporarily set `null`
        TestChromosomeCrossover crossover = new TestChromosomeCrossover();
    
        // Create the chromosome generator
        TestChromosomeGenerator generator = new TestChromosomeGenerator(testGenerationTarget, mutation, crossover);
        mutation.setGenerator(generator); // Set the generator after initialization
    
        // Create the branch-to-fitness function map
        Map<IBranch, FitnessFunction<TestChromosome>> branchToFitness = new HashMap<>();
        for (IBranch branch : branchesToCover) {
            branchToFitness.put(branch, new BranchCoverageFitnessFunction(testGenerationTarget));
        }
    
        // Return the RandomSearch algorithm
