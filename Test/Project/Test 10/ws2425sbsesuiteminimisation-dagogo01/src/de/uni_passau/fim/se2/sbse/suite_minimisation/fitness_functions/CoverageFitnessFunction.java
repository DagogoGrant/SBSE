...
public double applyAsDouble(TestSuiteChromosome chromosome) {
    ...
    // Shortcut for fully covered test suites
    if (coveredCount == totalLines) {
        return 1.0;
    }
    ...
    // Compute fitness with proper weighting
    double fitness = Math.min(1.0, weightCoefficient * normalizedCoverage + (1.0 - weightCoefficient) * normalizedWeight);
