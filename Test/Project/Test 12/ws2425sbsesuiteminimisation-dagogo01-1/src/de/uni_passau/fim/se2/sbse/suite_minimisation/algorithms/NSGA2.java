...
public class NSGA2 implements GeneticAlgorithm<TestSuiteChromosome> {
    ...
    public List<TestSuiteChromosome> findSolution() {
        ...
        // Step 3: Iteratively improve  population
    ...
    protected void calculateCrowdingDistances(List<List<TestSuiteChromosome>> fronts) {
        for (List<TestSuiteChromosome> front : fronts) {
            ...
            int numObjectives = 2; // Assume 2 objectives
            int frontSize = front.size();
    
            for (int i = 0; i < numObjectives; i++) {
                int finalI = i;
    
                // Create a mutable copy of the front for sorting
                List<TestSuiteChromosome> sortedFront = new ArrayList<>(front);
    
                // Sort front based on the current objective
                sortedFront.sort((a, b) -> Double.compare(a.getObjective(finalI), b.getObjective(finalI)));
    
                // Avoid duplicate invocations for first and last chromosomes
                if (sortedFront.get(0).getCrowdingDistance() != Double.POSITIVE_INFINITY) {
                    sortedFront.get(0).setCrowdingDistance(Double.POSITIVE_INFINITY);
                }
                if (sortedFront.get(frontSize - 1).getCrowdingDistance() != Double.POSITIVE_INFINITY) {
                    sortedFront.get(frontSize - 1).setCrowdingDistance(Double.POSITIVE_INFINITY);
                }
    
                double minObjective = sortedFront.get(0).getObjective(finalI);
                double maxObjective = sortedFront.get(frontSize - 1).getObjective(finalI);
                double range = maxObjective - minObjective;
    
                if (range == 0) continue; // Avoid division by zero
    
                for (int j = 1; j < frontSize - 1; j++) {
                    double distance = sortedFront.get(j).getCrowdingDistance();
                    distance += (sortedFront.get(j + 1).getObjective(finalI) - sortedFront.get(j - 1).getObjective(finalI)) / range;
                    sortedFront.get(j).setCrowdingDistance(distance);
                }
            }
        }
