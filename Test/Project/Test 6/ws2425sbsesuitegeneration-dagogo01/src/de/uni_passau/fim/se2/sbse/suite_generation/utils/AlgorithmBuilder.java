...
import de.uni_passau.fim.se2.sbse.suite_generation.crossover.TestCrossover;
import de.uni_passau.fim.se2.sbse.suite_generation.mutation.TestMutation;
...
public class AlgorithmBuilder {
  ...
  private GeneticAlgorithm<?> buildRandomSearch() {
    // Instantiate your specific crossover and mutation implementations
    TestCrossover crossover = new TestCrossover(random);
    TestMutation mutation = new TestMutation(
        random,
        Utils.allStatements(testGenerationTarget) // Collect all statements for mutation
    );

    // Instantiate the chromosome generator
    ...
    // Convert branches to a compatible type
    ...
    // Create the RandomSearch algorithm
    RandomSearch algorithm = new RandomSearch(
        stoppingCondition, // Stopping condition to control the algorithm
        generator,         // Generator for initializing the population
        targetBranches,    // Branches the algorithm aims to cover
