...
import de.uni_passau.fim.se2.sbse.suite_minimisation.algorithms.NSGAII;
...
public class AlgorithmBuilder {
    ...
    private MinimizingFitnessFunction<? extends Chromosome<?>> makeTestSuiteSizeFitnessFunction() {
        // Instantiate the TestSuiteSizeFitnessFunction
    ...
    private MaximizingFitnessFunction<? extends Chromosome<?>> makeTestSuiteCoverageFitnessFunction() {
        // Instantiate the TestSuiteCoverageFitnessFunction with coverageMatrix
        return new TestSuiteCoverageFitnessFunction(coverageMatrix);
    }
...
private GeneticAlgorithm<? extends Chromosome<?>> buildNSGA2() {
    int populationSize = 100; // Hardcoded population size; adjust as needed
    // Create mutation and crossover operators
    Mutation<TestSuiteChromosome> mutation = new TestSuiteMutation(0.05); // 5% mutation probability
    Crossover<TestSuiteChromosome> crossover = new TestSuiteCrossover();

    // Create the chromosome generator
    ...
    FitnessFunction<TestSuiteChromosome> sizeFitnessFunction = new TestSuiteSizeFitnessFunction();
    ...
    BinaryTournamentSelection<TestSuiteChromosome> selection = new BinaryTournamentSelection<>(
            Comparator.comparingDouble(TestSuiteChromosome::getSizeFitness), // Compare by size fitness
            random
    );

    // Instantiate and return the NSGAII algorithm
    return new NSGAII(
            generator,
            sizeFitnessFunction,
            ...
            crossover,
            mutation,
            populationSize // Replace with the desired population size
    );
}
...
private GeneticAlgorithm<? extends Chromosome<?>> buildRandomSearch() {
    // Instantiate mutation and crossover operators
    Mutation<TestSuiteChromosome> mutation = new TestSuiteMutation(0.1); // Mutation probability = 0.1
    Crossover<TestSuiteChromosome> crossover = new TestSuiteCrossover();
    ...
    // Instantiate the fitness functions
    FitnessFunction<TestSuiteChromosome> sizeFitnessFunction = new TestSuiteSizeFitnessFunction();
    FitnessFunction<TestSuiteChromosome> coverageFitnessFunction = new TestSuiteCoverageFitnessFunction(coverageMatrix);

    // Return the RandomSearch instance
