...
public class Utils {
    ...
    public static double computeHyperVolume(
        ...
        // Sort the Pareto front based on the first fitness function (f1) in ascending order
        List<Object> sortedFront = new ArrayList<>(front);
        sortedFront.sort(Comparator.comparingDouble(c -> f1.applyAsDouble(c)));
    
        double hyperVolume = 0.0;
        double previousF1 = r1; // Initialize to the reference point's r1
        double currentF2;
    
        System.out.println("Debugging computeHyperVolume:");
        System.out.printf("Reference Points: r1 = %.3f, r2 = %.3f%n", r1, r2);
        System.out.println("Sorted Front (based on F1):");
    
        for (Object chromosome : sortedFront) {
            double currentF1 = f1.applyAsDouble(chromosome);
            currentF2 = f2.applyAsDouble(chromosome);
    
            // Log fitness values for the chromosome
            System.out.printf("Chromosome F1: %.6f, F2: %.6f%n", currentF1, currentF2);
    
            // Calculate the rectangular area from (previousF1, r2) to (currentF1, currentF2)
            ...
            if (width > 0 && height > 0) {
                ...
                // Apply rounding to reduce floating-point precision errors
                double roundedArea = Math.round(area * 1e12) / 1e12;
    
                hyperVolume += roundedArea;
    
                // Log area contribution to hyperVolume
                System.out.printf("Width: %.6f, Height: %.6f, Area: %.6f, Rounded Area: %.6f, Cumulative HyperVolume: %.6f%n",
                                  width, height, area, roundedArea, hyperVolume);
            } else {
                System.out.println("Skipping invalid rectangle (negative width or height).");
            }
    
            // Update previousF1 for the next iteration
            previousF1 = currentF1;
        }
    
        System.out.printf("Final HyperVolume: %.6f%n", hyperVolume);
        return hyperVolume;
    }
