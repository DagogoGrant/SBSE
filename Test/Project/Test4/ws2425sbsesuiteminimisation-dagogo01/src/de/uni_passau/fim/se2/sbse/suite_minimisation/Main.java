...
List<String> getTestCaseNamesFrom(final Chromosome<?> testSuiteChromosome) {
    if (!(testSuiteChromosome instanceof TestSuiteChromosome chromosome)) {
        throw new IllegalArgumentException("Invalid chromosome type.");
    }
    ...
    for (int i = 0; i < testCases.length; i++) {
        if (testCases[i]) {
            activeTestCaseNames.add(testCases[i] ? this.testCases[i] : null);
        }
    }
...
double getSizeOf(final Chromosome<?> testSuiteChromosome) {
    ...
    for (boolean testCase : chromosome.getTestCases()) {
        if (testCase) activeTests++;
    }

    return (double) activeTests / TestSuiteChromosome.getTotalTestCases();
}
