...
public final class CoverageTracker {
    ...
    private void measureCoverage() throws Exception {
        for (int i = 0; i < testCases.length; i++) {
            ...
            // Debugging log
            System.out.printf("Test case '%s' covers lines: %s%n", testCase, Arrays.toString(coverage));
        }
    
        // Log overall coverage matrix
        System.out.println("Final Coverage Matrix:");
        for (int i = 0; i < coverageMatrix.length; i++) {
            System.out.printf("Test Case %s: %s%n", testCases[i], Arrays.toString(coverageMatrix[i]));
        }
    }
