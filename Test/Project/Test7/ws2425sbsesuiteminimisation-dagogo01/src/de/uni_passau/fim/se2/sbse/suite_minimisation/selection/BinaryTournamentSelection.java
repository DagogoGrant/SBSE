...
public C apply(final List<C> population) throws NullPointerException, NoSuchElementException {
    ...
    // Shuffle the population to ensure random selection
    ...
    C best = shuffledPopulation.get(0);
    for (int i = 1; i < Math.min(TOURNAMENT_SIZE, shuffledPopulation.size()); i++) {
        C competitor = shuffledPopulation.get(i);
        if (comparator.compare(competitor, best) > 0) {
            best = competitor;
        }
    }
    return best;
