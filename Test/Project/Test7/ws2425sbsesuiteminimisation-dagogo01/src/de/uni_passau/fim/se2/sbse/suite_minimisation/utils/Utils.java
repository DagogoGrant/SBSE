...
public class Utils {
    ...
    public static double computeHyperVolume(
        ...
        for (Object chromosome : sortedFront) {
            ...
            if (currentF1 > r1 || currentF2 > r2) {
                throw new IllegalArgumentException("Fitness values exceed reference points.");
            }
    
            // Calculate the rectangular area from (previousF1, r2) to (currentF1, currentF2)
            double width = previousF1 - currentF1; // Corrected to ensure valid width
            double height = currentF2 - r2;       // Corrected to ensure valid height
    
            if (width > 0 && height < 0) { // Height is negative because r2 is smaller
                hyperVolume += width * Math.abs(height);
            }
