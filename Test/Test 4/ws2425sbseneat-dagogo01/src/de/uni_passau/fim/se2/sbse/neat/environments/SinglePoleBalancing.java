...
public class SinglePoleBalancing implements Environment {
    ...
    private double singleEvaluation(Agent agent) {
        ...
        while (!isDone()) {
            List<Double> state = getState();
            List<Double> action = agent.getOutput(state);
            updateState(action);

            // Calculate step fitness components
            double angleComponent = 1.0 - (Math.abs(poleAngle) / MAX_ANGLE);
            double positionComponent = 1.0 - (Math.abs(cartPosition) / MAX_POSITION);
            double velocityComponent = 1.0 - (Math.abs(cartVelocity) / MAX_VELOCITY);
            double angularVelocityComponent = 1.0 - (Math.abs(poleAngularVelocity) / MAX_POLE_VELOCITY);

            // Weighted sum of components
            double stepFitness = angleComponent * 0.4 +
                    positionComponent * 0.3 +
                    velocityComponent * 0.15 +
                    angularVelocityComponent * 0.15;

            // Add step fitness with time bonus
            accumulatedFitness += stepFitness * (1.0 + steps / (double)maxSteps);
        }

        // Add completion bonus if not failed
        if (!failed) {
            accumulatedFitness *= 1.5;
        }
    ...
    @Override
public double evaluateAgent(Agent agent) {
    double totalFitness = 0.0;

    // Run multiple simulations (to get a robust fitness estimate)
    for (int i = 0; i < repeats; i++) {
        totalFitness += singleEvaluation(agent);
    }

    return totalFitness / repeats;  // Normalize fitness over multiple runs
}
