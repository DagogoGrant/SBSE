...
public double evaluateAgent(Agent agent) {
    ...
    for (int i = 0; i < XOR_INPUTS.length; i++) {
        List<Double> input = List.of(XOR_INPUTS[i][0], XOR_INPUTS[i][1]);
        List<Double> output = agent.getOutput(input);
        double error = Math.abs(XOR_OUTPUTS[i] - output.get(0));
        fitness += 1 - error;  // Higher fitness for closer outputs
    }
    return fitness * 4; // Scale fitness to [0, 16]
}
