...
public class Main implements Callable<Integer> {
     ...
     */
    public Integer call() {
        // Set seed for deterministic random behavior
        Randomness.random().setSeed(42);
        System.out.println("Seed set to 42 for deterministic behavior.");
        ...
        AlgorithmBuilder builder = new AlgorithmBuilder(
            Randomness.random(),
            new MaxFitnessEvaluations(maxEvaluations),
            populationSize,
            className,
            packageName,
            BranchTracer.getInstance()
        );
