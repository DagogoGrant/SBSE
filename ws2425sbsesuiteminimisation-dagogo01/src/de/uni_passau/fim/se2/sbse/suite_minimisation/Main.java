...
List<String> getTestCaseNamesFrom(final Chromosome<?> testSuiteChromosome) {
    if (!(testSuiteChromosome instanceof TestSuiteChromosome)) {
        throw new IllegalArgumentException("Expected a TestSuiteChromosome");
    }

    // Cast the chromosome to the correct type
    TestSuiteChromosome chromosome = (TestSuiteChromosome) testSuiteChromosome;

    // Extract the test case names based on the selected test cases
    List<String> selectedTestCaseNames = new ArrayList<>();
    boolean[] testCases = chromosome.getTestCases();
    for (int i = 0; i < testCases.length; i++) {
        if (testCases[i]) { // Check if the test case is selected
            selectedTestCaseNames.add(this.testCases[i]);
        }
    }
    return selectedTestCaseNames;
}
...
double getCoverageOf(final Chromosome<?> testSuiteChromosome) {
    ...
    // Compute normalized coverage using the coverage fitness function
    try {
        return chromosome.computeCoverageFitness();
    } catch (Exception e) {
        throw new RuntimeException("Error computing coverage fitness", e);
    }
}
    ...
    double getSizeOf(final Chromosome<?> testSuiteChromosome) {
        ...
        // Compute normalized size using the size fitness function
        return chromosome.computeSizeFitness();
    }
